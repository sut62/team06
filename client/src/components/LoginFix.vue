  ...
  <v-app id="login"  :style="{ backgroundImage: 'url(' + require('@/assets/BGLOG.jpg') + ')' }">
    <v-content>
      <v-container
              fluid
              class="hbg"
              id="customer"  :style="{ backgroundImage: 'url(' + require('@/assets/lfix.png') + ')' }"
      >
    ...
    <v-snackbar  top v-model="snaktn" :timeout="10000">{{snactexttrue}}
      <v-btn text @click="snaktn = false" to="/fix">OK</v-btn>
    </v-snackbar>
      ...
      UserValidate() {
        ...
        http
                ...
                .then(response => {
                  ...
                  else if(this.message == '{"fix":"false"}'){
                    this.snaktr = true;
